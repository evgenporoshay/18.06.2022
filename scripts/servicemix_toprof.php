...
$query1 = mysqli_query($link, "SELECT UF_CRM_1650003076612
    ...
    WHERE bcm.STAGE_ID = 'C22:NEW' /* Тут мы делаем запрос на стадии новой заявки СТП */
...
$query4 = mysqli_query($link, "SELECT bcd.ID
    FROM b_crm_deal bcd
    WHERE bcd.STAGE_ID = 'C22:NEW' /* Тут мы делаем запрос на стадии новой заявки СТП */
...
$query8 = mysqli_query($link, "SELECT data_provedeniya 
from toprof t 
where carriage = '$carriage'
order by id desc
limit 1 ");

while($row8 = $query8->fetch_assoc())

$data_provedeniya .= $row8['data_provedeniya'];//тут мы получили филиал вагона $carriage

mysqli_query($link, "UPDATE b_uts_crm_deal set UF_CRM_1654068975186 = '$data_provedeniya' where UF_CRM_1650018274186 = '$deal_id' limit 1");


$query9 = mysqli_query($link, "SELECT engeneer 
from toprof t 
where carriage = '$carriage'
order by id desc
limit 1 ");
...
$engeneer .= $row9['engeneer'];//тут мы получили филиал вагона $carriage

mysqli_query($link, "UPDATE b_uts_crm_deal set UF_CRM_1654068360080 = '$engeneer' where UF_CRM_1650018274186 = '$deal_id' limit 1");




$query11 = mysqli_query($link, "SELECT telephone 
from toprof t 
where carriage = '$carriage'
order by id desc
limit 1 ");
...
$telephone .= $row11['telephone'];//тут мы получили филиал вагона $carriage


mysqli_query($link, "UPDATE b_uts_crm_deal set UF_CRM_1654077567877 = '$telephone' where UF_CRM_1650018274186 = '$deal_id' limit 1");


$query5 = mysqli_query($link, "SELECT skdu from toprof_cars where carriage = '$carriage_new' ");
while($row5 = $query5->fetch_assoc())
$skdu .= $row5['skdu'];//тут мы получили год постройки вагона $carriage


$query6 = mysqli_query($link, "SELECT satellite_connection from toprof_cars where carriage = '$carriage_new'");
while($row6 = $query6->fetch_assoc())
$satellite_connection .= $row6['satellite_connection'];


$query7 = mysqli_query($link, "SELECT it_service FROM toprof_cars WHERE carriage = '$carriage_new' ");
while($row7 = $query7->fetch_assoc())
$it_service .= $row7['it_service'];


$query8 = mysqli_query($link, "SELECT video_system FROM toprof_cars WHERE carriage = '$carriage_new' ");
while($row8 = $query8->fetch_assoc())
$video_system .= $row8['video_system'];


if ($skdu == '1') {

mysqli_query($link, "INSERT INTO b_crm_product_row
(ID, OWNER_ID, OWNER_TYPE, PRODUCT_ID, PRODUCT_NAME, PRICE, PRICE_ACCOUNT, PRICE_EXCLUSIVE, PRICE_NETTO, PRICE_BRUTTO, QUANTITY, DISCOUNT_TYPE_ID, DISCOUNT_RATE, DISCOUNT_SUM, TAX_RATE, TAX_INCLUDED, CUSTOMIZED, MEASURE_CODE, MEASURE_NAME, SORT)
VALUES(null, '$deal_id', 'D', 5148, '', 660, 660, 550, 550, 660, 1, 2, 0, 0, 20, 'N', 'Y', 796, 'шт',null) ");

mysqli_query($link, "UPDATE b_crm_deal set OPPORTUNITY = (select SUM(PRICE) from b_crm_product_row bcpr where OWNER_ID ='$deal_id') where id = '$deal_id' limit 1");

mysqli_query($link, "UPDATE b_crm_deal set TAX_VALUE = (select sum(price)-sum(price_netto) from b_crm_product_row bcpr where OWNER_ID = '$deal_id') where id = '$deal_id' limit 1");

mysqli_query($link, "UPDATE b_crm_deal set OPPORTUNITY_ACCOUNT = (select SUM(PRICE) from b_crm_product_row bcpr where OWNER_ID ='$deal_id')  where id = '$deal_id' limit 1");

mysqli_query($link, "UPDATE b_crm_deal set CURRENCY_ID = 'RUB' where id = '$deal_id' limit 1");

mysqli_query($link, "UPDATE b_crm_deal set TAX_VALUE_ACCOUNT = (select sum(price)-sum(price_netto) from b_crm_product_row bcpr where OWNER_ID = '$deal_id')  where id = '$deal_id' limit 1");

mysqli_query($link, "UPDATE b_crm_deal set ACCOUNT_CURRENCY_ID = 'RUB' where id = '$deal_id' limit 1");
...
mysqli_query($link, "INSERT INTO b_crm_product_row
(ID, OWNER_ID, OWNER_TYPE, PRODUCT_ID, PRODUCT_NAME, PRICE, PRICE_ACCOUNT, PRICE_EXCLUSIVE, PRICE_NETTO, PRICE_BRUTTO, QUANTITY, DISCOUNT_TYPE_ID, DISCOUNT_RATE, DISCOUNT_SUM, TAX_RATE, TAX_INCLUDED, CUSTOMIZED, MEASURE_CODE, MEASURE_NAME, SORT)
VALUES(null, '$deal_id', 'D', 5151, '', 1740, 1740, 1450, 1450, 1740, 1, 2, 0, 0, 20, 'N', 'Y', 796, 'шт',null)");
...
mysqli_query($link, "INSERT INTO b_crm_product_row
(ID, OWNER_ID, OWNER_TYPE, PRODUCT_ID, PRODUCT_NAME, PRICE, PRICE_ACCOUNT, PRICE_EXCLUSIVE, PRICE_NETTO, PRICE_BRUTTO, QUANTITY, DISCOUNT_TYPE_ID, DISCOUNT_RATE, DISCOUNT_SUM, TAX_RATE, TAX_INCLUDED, CUSTOMIZED, MEASURE_CODE, MEASURE_NAME, SORT)
VALUES(null, '$deal_id', 'D', 5149, '', 300, 300, 250, 250, 660, 1, 2, 0, 0, 20, 'N', 'Y', 796, 'шт',null)");
...
if ($satellite_connection == '1') {

mysqli_query($link, "INSERT INTO b_crm_product_row
(ID, OWNER_ID, OWNER_TYPE, PRODUCT_ID, PRODUCT_NAME, PRICE, PRICE_ACCOUNT, PRICE_EXCLUSIVE, PRICE_NETTO, PRICE_BRUTTO, QUANTITY, DISCOUNT_TYPE_ID, DISCOUNT_RATE, DISCOUNT_SUM, TAX_RATE, TAX_INCLUDED, CUSTOMIZED, MEASURE_CODE, MEASURE_NAME, SORT)
VALUES(null, '$deal_id', 'D', 5150, '', 1200, 1200, 1000, 1000, 1200, 1, 2, 0, 0, 20, 'N', 'Y', 796, 'шт',null)");
...
//mysqli_query($link, "UPDATE b_uts_crm_deal SET UF_CRM_1648379964086 = '$skdu' WHERE b_uts_crm_deal.VALUE_ID = '$deal_id' LIMIT 1 ");


switch ($filial) {
      case 'С-ЗАП':
         mysqli_query($link, "UPDATE b_crm_deal, b_uts_crm_deal SET UF_CRM_1650180313032 = 73,  b_crm_deal.ASSIGNED_BY_ID = 24 WHERE b_crm_deal.ID = b_uts_crm_deal.VALUE_ID AND b_uts_crm_deal.VALUE_ID = '$deal_id' ");
      break;
      case 'С-КВ': 
         mysqli_query($link, "UPDATE b_crm_deal, b_uts_crm_deal SET UF_CRM_1650180313032 = 74,  b_crm_deal.ASSIGNED_BY_ID = 22 WHERE b_crm_deal.ID = b_uts_crm_deal.VALUE_ID AND b_uts_crm_deal.VALUE_ID = '$deal_id' ");
      break;
      case 'КБШ': 
          mysqli_query($link, "UPDATE b_crm_deal, b_uts_crm_deal SET UF_CRM_1650180313032 = 75,  b_crm_deal.ASSIGNED_BY_ID = 20 WHERE b_crm_deal.ID = b_uts_crm_deal.VALUE_ID AND b_uts_crm_deal.VALUE_ID = '$deal_id' ");
      break;
      case 'З-СИБ':
          mysqli_query($link, "UPDATE b_crm_deal, b_uts_crm_deal SET UF_CRM_1650180313032 = 76,  b_crm_deal.ASSIGNED_BY_ID = 25 WHERE b_crm_deal.ID = b_uts_crm_deal.VALUE_ID AND b_uts_crm_deal.VALUE_ID = '$deal_id' ");
      break;
      case 'УРАЛ':
             mysqli_query($link, "UPDATE b_crm_deal, b_uts_crm_deal SET UF_CRM_1650180313032 = 77,  b_crm_deal.ASSIGNED_BY_ID = 19 WHERE b_crm_deal.ID = b_uts_crm_deal.VALUE_ID AND b_uts_crm_deal.VALUE_ID = '$deal_id' ");
      break;
      case 'МОСК':
            mysqli_query($link, "UPDATE b_crm_deal, b_uts_crm_deal SET UF_CRM_1650180313032 = 78,  b_crm_deal.ASSIGNED_BY_ID = 26 WHERE b_crm_deal.ID = b_uts_crm_deal.VALUE_ID AND b_uts_crm_deal.VALUE_ID = '$deal_id' ");
      break;
      case 'СЕВ':
            mysqli_query($link, "UPDATE b_crm_deal, b_uts_crm_deal SET UF_CRM_1650180313032 = 79,  b_crm_deal.ASSIGNED_BY_ID = 24 WHERE b_crm_deal.ID = b_uts_crm_deal.VALUE_ID AND b_uts_crm_deal.VALUE_ID = '$deal_id' ");
      break;
      case 'ДВОСТ':
             mysqli_query($link, "UPDATE b_crm_deal, b_uts_crm_deal SET UF_CRM_1650180313032 = 80,  b_crm_deal.ASSIGNED_BY_ID = 92 WHERE b_crm_deal.ID = b_uts_crm_deal.VALUE_ID AND b_uts_crm_deal.VALUE_ID = '$deal_id' ");
      break;
      case 'ГОР':
             mysqli_query($link, "UPDATE b_crm_deal, b_uts_crm_deal SET UF_CRM_1650180313032 = 81,  b_crm_deal.ASSIGNED_BY_ID = 97 WHERE b_crm_deal.ID = b_uts_crm_deal.VALUE_ID AND b_uts_crm_deal.VALUE_ID = '$deal_id' ");
      break;
      case 'В-СИБ':
             mysqli_query($link, "UPDATE b_crm_deal, b_uts_crm_deal SET UF_CRM_1650180313032 = 82,  b_crm_deal.ASSIGNED_BY_ID = 25 WHERE b_crm_deal.ID = b_uts_crm_deal.VALUE_ID AND b_uts_crm_deal.VALUE_ID = '$deal_id' ");
      break;
      case 'ПРИВ':
             mysqli_query($link, "UPDATE b_crm_deal, b_uts_crm_deal SET UF_CRM_1650180313032 = 83,  b_crm_deal.ASSIGNED_BY_ID = 21 WHERE b_crm_deal.ID = b_uts_crm_deal.VALUE_ID AND b_uts_crm_deal.VALUE_ID = '$deal_id' ");
      break;    
      case 'ЗБК':
             mysqli_query($link, "UPDATE b_crm_deal, b_uts_crm_deal SET UF_CRM_1650180313032 = 84,  b_crm_deal.ASSIGNED_BY_ID = 25 WHERE b_crm_deal.ID = b_uts_crm_deal.VALUE_ID AND b_uts_crm_deal.VALUE_ID = '$deal_id' ");
      break;
       case 'ЕН':
             mysqli_query($link, "UPDATE b_crm_deal, b_uts_crm_deal SET UF_CRM_1650180313032 = 85,  b_crm_deal.ASSIGNED_BY_ID = 25 WHERE b_crm_deal.ID = b_uts_crm_deal.VALUE_ID AND b_uts_crm_deal.VALUE_ID = '$deal_id' ");
      break;
...
set STAGE_ID = 'C22:UC_CISU50' where '$carriage_new' in (select carriage from toprof_cars tc where tc.it_service = '0' and tc.satellite_connection = '0' and tc.skdu = '0' and tc.video_system = '0')
and bcd.ID = '$deal_id'");
