...
$query1 = mysqli_query($link, "
    ...
    WHERE bcm.STAGE_ID = 'C11:NEW' /* Тут мы делаем запрос на стадии новой заявки СТП */
    AND bcm.DATE_CREATE >= DATE_SUB(NOW(), INTERVAL 30 SECOND);");
...
$query4 = mysqli_query($link, "SELECT bcd.ID
    FROM b_crm_deal bcd
    WHERE bcd.STAGE_ID = 'C11:NEW' /* Тут мы делаем запрос на стадии новой заявки СТП */
...
switch ($filial) {
      ...
      break;
      case 'ГОР':
             mysqli_query($link, "UPDATE b_crm_deal, b_uts_crm_deal SET UF_CRM_1650180313032 = 81,  b_crm_deal.ASSIGNED_BY_ID = 83 WHERE b_crm_deal.ID = b_uts_crm_deal.VALUE_ID AND b_uts_crm_deal.VALUE_ID = '$deal_id' ");
      break;
      case 'В-СИБ':
