...
bui.USER_ID = bu.ID
WHERE DATE_ADD(bu.PERSONAL_BIRTHDAY, INTERVAL YEAR(CURDATE())-YEAR(bu.PERSONAL_BIRTHDAY) + IF(DAYOFYEAR(CURDATE()) > DAYOFYEAR(bu.PERSONAL_BIRTHDAY), 1, 0) YEAR) = DATE_ADD(CURDATE(), INTERVAL 10 day)
AND UF_DEPARTMENT_NAME = 'Поддержка Попутчик' AND bu.ACTIVE = 'Y'"); // запрос на выборку
$count = mysqli_num_rows($result); // количество строк в запросе
$title .= '<b>Уважаемый Александр Борисович! </b><br /> <p>Через 10 дней у следующих сотрудников вашего отдела будет День Рождения! <br /> Не забудьте их поздравить!<br /></p>';
...
$to  = "a.sukhostavskiy@rdl-telecom.com," ;
